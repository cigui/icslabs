...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    switch (op) {
      ...
      case A_SUB:
        ...
        if ((argB > 0 && argA < 0 && val < 0)||(argB < 0 && argA > 0 && val > 0))
          ovf = TRUE;
        break;
      case A_AND:
        if (val == 0) zero = TRUE;
...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RRMOVL:{  /* 2:x regA:regB */
        ...
        regid_t rA = GET_REGA(regs);
        regid_t rB = GET_REGB(regs);
        if (cond_doit(sim->cc, ifun) == TRUE){
            long_t val = get_reg_val(sim->r, rA);
      ...
      case I_IRMOVL:{ /* 3:0 F:regB imm */
        ...
        regid_t rB = GET_REGB(regs);
        long_t valC = 0;
        if (!get_long_val(sim->m, next_pc+1, &valC)){
            err_print("PC = 0x%x, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
      ...
      case I_JMP: { /* 7:x imm */
        ...
        next_pc += 4;
        bool_t Cnd = cond_doit(sim->cc, ifun);
        next_pc = Cnd? valC : next_pc;
        sim->pc = next_pc;
        break;
      }
      case I_CALL: { /* 8:x imm */
        ...
        long_t valB = get_reg_val(sim->r, REG_ESP);
        long_t valE = valB - 4;
        next_pc += 4;
        set_long_val(sim->m, valE, next_pc);
        set_reg_val(sim->r, REG_ESP, valE);
        long_t check = 0;
        if (!get_long_val(sim->m, valE, &check)){
      ...
      case I_RET: { /* 9:0 */
        long_t valA = get_reg_val(sim->r, REG_ESP);
        long_t valB = get_reg_val(sim->r, REG_ESP);
        long_t valE = valB + 4;
        long_t valM = 0;
        if (!get_long_val(sim->m, valA, &valM)){
            err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, valM);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_ESP, valE);
        next_pc = valM;
      ...
      case I_POPL: { /* B:0 regA:F */
        ...
        if (!get_long_val(sim->m, valB, &valM)){
            err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, valB);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_ESP, valE);
        set_reg_val(sim->r, rA, valM);
